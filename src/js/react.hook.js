...
export function HookEx() {
    ...
    // # useState(): 傳入 state 起始值(可為 function 經複雜邏輯計算後回傳起始值)，
    ...
    // # setCount: 不像 class this.setState 會合併原本的 state object，hook 更新 state 變數會直接取代，
    //   除非利用 setState(prevState => {return {...prevState, ...updatedValues}})。
    ...
    //   傳入的第二個可選參數 array 如下僅在計數更改時才重新執行 effect。
    // # array: 包含 component 內隨時間變化並被 effect 用到的值(props 或 state)，
    //   若未完全包含 effect 用到的所有值，則未包含的部分會引用先前 render 的舊變數值。
    //   傳遞空 array([])，表示 effect 不依賴 props 或 state，因此不需重新執行，僅在 mount 執行一次 和 unmount 清除一次，
    //   effect 內部的 props 和 state 會一直為初始值。
